...
const EpisodeDetails: React.FC<EpisodeDetailsProps> = ({ episode, patient, visits, onNewVisit, onUpdateEpisode, role, onSendReferral, onOpenPresentation }) => {
   ...
   const [isEditingVascular, setIsEditingVascular] = useState(false);
   const [isEditingOffloading, setIsEditingOffloading] = useState(false);
   const [vascularForm, setVascularForm] = useState(episode.vascularStatus);
   const [offloadingForm, setOffloadingForm] = useState(episode.offloading || '');
   ...
   const handleSaveVascular = () => {
      onUpdateEpisode({ ...episode, vascularStatus: vascularForm });
      setIsEditingVascular(false);
   };

   const handleSaveOffloading = () => {
      onUpdateEpisode({ ...episode, offloading: offloadingForm });
      setIsEditingOffloading(false);
   };
   ...
   return (
      <div className="space-y-6">
         ...
         <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div className={`p-4 rounded-xl border-2 flex flex-col items-center justify-center text-center ${wifi.amputationRisk === 'Alto' ? 'bg-rose-50 border-rose-200' :
               ...
               <span className={`text-sm font-black ${wifi.amputationRisk === 'Alto' ? 'text-rose-700' :
         ...
         <div className="border-b border-slate-200 flex gap-6 overflow-x-auto">
            {[
               ...
               <button
                  ...
                  className={`pb-3 text-sm font-bold flex items-center gap-2 whitespace-nowrap border-b-2 transition-all ${activeTab === tab.id ? 'border-blue-500 text-blue-600' : 'border-transparent text-slate-400 hover:text-slate-600'
                     }`}
               >
                  ...
                  {role === UserRole.VASCULAR && (
                     <div className="space-y-6">
                        <div className="flex justify-between items-center">
                           ...
                           {!isEditingVascular && (
                              <button onClick={() => setIsEditingVascular(true)} className="text-sm text-blue-600 font-bold hover:bg-blue-50 px-3 py-1 rounded-lg transition-colors">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                 <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                                    ...
                                    <input
                                       ...
                                       value={vascularForm?.abi || ''}
                                       onChange={e => setVascularForm({ ...vascularForm, abi: Number(e.target.value) })}
                                    />
                                 ...
                                 <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                                    ...
                                    <select
                                       className="w-full p-2 border rounded text-sm font-bold"
                                       value={vascularForm?.revascularizable}
                                       onChange={e => setVascularForm({ ...vascularForm, revascularizable: e.target.value as any })}
                                    >
                                 ...
                                 <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                       <input
                                          ...
                                          value={vascularForm?.pulses?.dp || ''}
                                          onChange={e => setVascularForm({ ...vascularForm, pulses: { ...vascularForm?.pulses, dp: e.target.value } })}
                                       />
                                       <input
                                          placeholder="PT" className="p-2 border rounded text-xs"
                                          value={vascularForm?.pulses?.pt || ''}
                                          onChange={e => setVascularForm({ ...vascularForm, pulses: { ...vascularForm?.pulses, pt: e.target.value } })}
                                       />
                                    </div>
                                 </div>
                              </div>
                              <div className="flex justify-end gap-2">
                                 <button type="button" onClick={() => setIsEditingVascular(false)} className="px-3 py-1 text-rose-700 font-bold text-xs">Cancelar</button>
                                 <button type="button" onClick={handleSaveVascular} className="px-3 py-1 bg-rose-600 text-white rounded font-bold text-xs">Guardar</button>
                              </div>
                           </div>
                        ) : (
                           <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                              <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                                 <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">Índice Tobillo-Brazo (ABI)</label>
                                 <p className="text-sm font-bold">{episode.vascularStatus?.abi || 'N/A'}</p>
                                 <p className="text-[10px] text-slate-400 mt-2 italic">Valores &lt; 0.5 sugieren isquemia crítica.</p>
                              </div>
                              <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                                 <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">Decisión Táctica</label>
                                 <p className="text-sm font-bold">{episode.vascularStatus?.revascularizable || 'N/A'}</p>
                              </div>
                              <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                                 <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">Pulsos DP / PT</label>
                                 <p className="text-sm font-bold">DP: {episode.vascularStatus?.pulses?.dp || 'N/A'}</p>
                                 <p className="text-sm font-bold">PT: {episode.vascularStatus?.pulses?.pt || 'N/A'}</p>
                              </div>
                           </div>
                        )}
                        <div>
